...
app.MapPost("/api/chilaquiles", async (HttpContext ctx, Dictionary<string, System.Text.Json.JsonElement> body) =>
{
    ...
    using var conn = CreateConnection(); await (conn as dynamic).OpenAsync();
    using var cmd = conn.CreateCommand();
    cmd.CommandText = "INSERT INTO chilaquiles(name,salsaType,protein,spiciness,price,createdAt,is_active) VALUES (?,?,?,?,?,NOW(),1)";
    var pn = cmd.CreateParameter(); pn.Value = name; pn.DbType = System.Data.DbType.String; cmd.Parameters.Add(pn);
    var ps = cmd.CreateParameter(); ps.Value = salsaType; ps.DbType = System.Data.DbType.String; cmd.Parameters.Add(ps);
    var pp = cmd.CreateParameter(); pp.Value = protein; pp.DbType = System.Data.DbType.String; cmd.Parameters.Add(pp);
    var psp = cmd.CreateParameter(); psp.Value = spiciness; psp.DbType = System.Data.DbType.Int32; cmd.Parameters.Add(psp);
    var ppr = cmd.CreateParameter(); ppr.Value = Convert.ToDouble(price); ppr.DbType = System.Data.DbType.Double; cmd.Parameters.Add(ppr);
    var affected = await (cmd as dynamic).ExecuteNonQueryAsync();
    if (affected == 1)
    {
...
app.MapPut("/api/chilaquiles/{id:int}", async (HttpContext ctx, int id, Dictionary<string, System.Text.Json.JsonElement> body) =>
{
    ...
    using var conn = CreateConnection(); await (conn as dynamic).OpenAsync();
    using var cmd = conn.CreateCommand();
    cmd.CommandText = "UPDATE chilaquiles SET name=?, salsaType=?, protein=?, spiciness=?, price=? WHERE id=?";
    var pn = cmd.CreateParameter(); pn.Value = name; pn.DbType = System.Data.DbType.String; cmd.Parameters.Add(pn);
    var ps = cmd.CreateParameter(); ps.Value = salsaType; ps.DbType = System.Data.DbType.String; cmd.Parameters.Add(ps);
    var pp = cmd.CreateParameter(); pp.Value = protein; pp.DbType = System.Data.DbType.String; cmd.Parameters.Add(pp);
    var psp = cmd.CreateParameter(); psp.Value = spiciness; psp.DbType = System.Data.DbType.Int32; cmd.Parameters.Add(psp);
    var ppr = cmd.CreateParameter(); ppr.Value = Convert.ToDouble(price); ppr.DbType = System.Data.DbType.Double; cmd.Parameters.Add(ppr);
    var pid = cmd.CreateParameter(); pid.Value = id; pid.DbType = System.Data.DbType.Int32; cmd.Parameters.Add(pid);
    var affected = await (cmd as dynamic).ExecuteNonQueryAsync();
    if (affected == 1) return Results.Ok(new { ok = true });
    return Results.NotFound();
});
...
app.MapDelete("/api/chilaquiles/{id:int}", async (HttpContext ctx, int id) =>
{
    try { RequireAuth(ctx); } catch { return Results.Unauthorized(); }
    using var conn = CreateConnection(); await (conn as dynamic).OpenAsync();
    using var cmd = conn.CreateCommand(); cmd.CommandText = "UPDATE chilaquiles SET is_active=0 WHERE id=?";
    var p = cmd.CreateParameter(); p.Value = id; cmd.Parameters.Add(p);
    var affected = await (cmd as dynamic).ExecuteNonQueryAsync();
    if (affected == 1) return Results.Ok(new { ok = true });
    return Results.NotFound();
...
app.MapPost("/api/chilaquiles/{id:int}/restore", async (HttpContext ctx, int id) =>
{
    try { RequireAuth(ctx); } catch { return Results.Unauthorized(); }
    using var conn = CreateConnection(); await (conn as dynamic).OpenAsync();
    using var cmd = conn.CreateCommand(); cmd.CommandText = "UPDATE chilaquiles SET is_active=1 WHERE id=?";
    var p = cmd.CreateParameter(); p.Value = id; cmd.Parameters.Add(p);
    var affected = await (cmd as dynamic).ExecuteNonQueryAsync();
    if (affected == 1) return Results.Ok(new { ok = true });
    return Results.NotFound();
